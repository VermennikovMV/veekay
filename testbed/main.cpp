...
veekay::mat4 orthographicMatrix(float left, float right, float bottom, float top,
...
veekay::mat4 lookAtMatrix(const veekay::vec3& eye, const veekay::vec3& target,
        ...
        veekay::vec3 up_dir = veekay::vec3::cross(forward, right);
        ...
        result[3][1] = -veekay::vec3::dot(up_dir, eye);
...
veekay::mat4 lightViewProjection() {
        ...
        const veekay::vec3 light_position = (-light_dir * 6.0f) + veekay::vec3{0.0f, 1.5f, 0.0f};
        const veekay::vec3 target{0.0f, -0.25f, 0.0f};

        veekay::mat4 view = lookAtMatrix(light_position, target, {0.0f, 1.0f, 0.0f});
        veekay::mat4 projection = orthographicMatrix(-5.5f, 5.5f, -5.5f, 5.5f, 0.1f, 12.0f);
...
inline namespace {
        ...
        VkExtent2D shadow_extent{shadow_map_size, shadow_map_size};
        bool shadow_map_ready_for_sampling = false;
}
...
bool createShadowMap(VkCommandBuffer cmd) {
        ...
        VkImageCreateInfo image_info{
                ...
                .extent = {shadow_extent.width, shadow_extent.height, 1},
                .mipLevels = 1,
                .arrayLayers = 1,
        ...
        for (uint32_t i = 0; i < properties.memoryTypeCount; ++i) {
                ...
                if ((requirements.memoryTypeBits & (1 << i)) &&
                    (type.propertyFlags & memory_flags) == memory_flags) {
                        memory_index = i;
                        break;
                }
        }
        ...
        shadow_map_ready_for_sampling = false;
...
void initialize(VkCommandBuffer cmd) {
	...
	{ // NOTE: Build graphics pipeline
                ...
                if (!shadow_vertex_shader_module) {
                        std::cerr << "Failed to load Vulkan shadow shader from file\n";
                        veekay::app.running = false;
                        return;
                }
		...
		// NOTE: Create graphics pipeline
                ...
                if (!createShadowMap(cmd)) {
                        veekay::app.running = false;
                        return;
                }
        }

        { // NOTE: Build shadow map pipeline
                ...
                VkViewport viewport{
                        .x = 0.0f,
                        .y = 0.0f,
                        .width = static_cast<float>(shadow_extent.width),
                        .height = static_cast<float>(shadow_extent.height),
                        .minDepth = 0.0f,
                        .maxDepth = 1.0f,
                };
                ...
                if (vkCreateGraphicsPipelines(device, nullptr, 1, &pipeline_info, nullptr,
                                              &shadow_pipeline) != VK_SUCCESS) {
                        std::cerr << "Failed to create shadow map pipeline\n";
                        veekay::app.running = false;
                        return;
                }
        }
	...
	scene_uniforms_buffer = new veekay::graphics::Buffer(
        ...
        scene_uniforms.light_view_projection = lightViewProjection();
...
void shutdown() {
        ...
        if (shadow_image_memory != VK_NULL_HANDLE) {
...
void render(VkCommandBuffer cmd, VkFramebuffer framebuffer) {
	vkResetCommandBuffer(cmd, 0);
        ...
        { // NOTE: Render scene into shadow map first
                VkImageMemoryBarrier to_depth{
                        .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
                        .srcAccessMask = shadow_map_ready_for_sampling ? VK_ACCESS_SHADER_READ_BIT : 0u,
                        .dstAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT,
                        .oldLayout = shadow_map_ready_for_sampling ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
                ...
                vkCmdPipelineBarrier(cmd,
                                     shadow_map_ready_for_sampling ? VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT
                                                                   : VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                     VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT,
                ...
                shadow_map_ready_for_sampling = true;
        }
